...
class Order:
    def __init__(self, config, currentOrderItr):
        self.id = None
        ...
        self.fillVolume = None
        self.initialPrice = None
        self.fillPrice = None
        self.config = config

    def id_generate(self, data_arr):
        id_arr = []
        for i in range(self.config.order_range):
    ...
    def change_date_generate(date, count):
        ...
        if dateTimestamp + diff in range(block1, block1 + timestampDiff):
            changedDate = str(
                datetime.fromtimestamp(dateTimestamp + diff + timestampDiff).replace(microsecond=microsecond))
